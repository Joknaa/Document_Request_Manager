...
public class DataBasePresenter {
    ...
    public static void SaveRequest(String cin, String apoge, String email, Documents docType) throws SQLException, ClassNotFoundException {
        Connect();
        String docTypeString = docType == Documents.Certificate ? "Certificate" : "Transcript";
        AddRequest(SQL_AddRequestAndGetID(apoge, email, docTypeString));
    ...
    public static String[] SQL_GetRequestDescription(int requestID) throws SQLException {
    ...
    public static String ManageRequest(String requestName, boolean accepted) throws SQLException, ClassNotFoundException {
        Connect();
        int id = GetRequestID(requestName);
        String Document = SQL_GetRequestDocument(id);
        SQL_ManageRequest(id, accepted);
        Disconnect();
        return Document;
    }

    private static String SQL_GetRequestDocument(int id) throws SQLException {
        String query = "SELECT Doc_type FROM request WHERE id = " + id + ";";
        ResultSet dataSet = Session.createStatement().executeQuery(query);
        dataSet.next();
    ...
    public static String GetRequestEmail(String requestName) throws SQLException, ClassNotFoundException {
        Connect();
        int id = GetRequestID(requestName);
        String query = "SELECT email FROM request WHERE id = " + id + ";";
        ResultSet dataSet = Session.createStatement().executeQuery(query);
        dataSet.next();
        String email = dataSet.getString(1);
        Disconnect();
        return email;
    }
